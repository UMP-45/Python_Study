...
if __name__ == "__main__":
    ...
    plt.sca(ax1)
    draw("RMSprop_train", x, train_acc_list, "epochs", "accuracy", "train acc", "red", ylim = 1.0)
    
    plt.sca(ax2)
    draw("RMSprop_test", x, test_acc_list, "epochs", "accuracy", "test acc", "green", ylim = 1.0,linestyle = '--')
    
    plt.sca(ax3)
    draw_both("RMSprop-拟合曲线", x, train_acc_list, test_acc_list, "epochs", "accuracy", "train acc", "test acc")

    plt.sca(ax4)
    draw("RMSprop_loss", x_loss, train_loss_list, "iters", "loss", "train_loss", "green")

    plt.tight_layout() #调整每个子图间的间距
    plt.savefig("RMSprop.jpg")
    plt.show()
