...
if __name__ == "__main__":
    ...
    x_loss = np.arange(len(train_loss_list))#[0:10]
    ...
    plt.sca(ax1)
    draw("SGD_train", x, train_acc_list, "epochs", "accuracy", "train acc", "red", ylim = 1.0)
    
    plt.sca(ax2)
    draw("SGD_test", x, test_acc_list, "epochs", "accuracy", "test acc", "green", ylim = 1.0, linestyle = '--')
    
    plt.sca(ax3)
    draw_both("SGD-拟合曲线", x, train_acc_list, test_acc_list, "epochs", "accuracy", "train acc", "test acc")

    plt.sca(ax4)
    draw("SGD_loss", x_loss, train_loss_list, "iters", "loss", "train_loss", "green")
